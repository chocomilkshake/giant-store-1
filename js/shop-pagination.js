document.addEventListener('DOMContentLoaded', function() {
    ...
    let filteredProducts = [...products]; // Initially, show all products
    ...
    const categoryFilters = document.querySelectorAll('.pixel-radio'); // Category filters

    // Check if elements exist before proceeding
    if (!productContainer || !pageInfo || !prevBtn || !nextBtn || !categoryFilters.length) {
    ...
    function displayProducts(page) {
        ...
        // Slice the filtered products array to get the relevant products for the page
        ...
        nextBtn.disabled = page === Math.ceil(filteredProducts.length / productsPerPage);
    }
    ...
    categoryFilters.forEach(filter => {
        filter.addEventListener('change', function() {
            const selectedCategoryId = this.id; // Get selected category ID
            filterProducts(selectedCategoryId); // Apply filter
        });
    });
